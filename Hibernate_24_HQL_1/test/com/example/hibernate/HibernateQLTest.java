...
import com.example.hibernate.Category;
import com.example.hibernate.Msg;
import com.example.hibernate.MsgInfo;
import com.example.hibernate.Topic;
...
//		q.setParameter(1, 8);
	...
	public void testHQL_12() {
		Session session = sf.openSession();
		session.beginTransaction();
		Query q = session.createQuery("select new com.example.hibernate.MsgInfo(m.id, m.cont, m.topic.title, m.topic.category.name) from Msg");
